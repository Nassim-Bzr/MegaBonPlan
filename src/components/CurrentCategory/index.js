...
export default function CurrentCategory() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/bonplans/category/${categoryId}`)
      ...
      .catch((error) =>
        console.error(
          'Erreur lors de la récupération des bons plans pour la catégorie:',
          error
        )
      )
  }, [categoryId])
  ...
  const submitNewBonPlan = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          responseData.message || 'Failed to upload the bon plan.'
        )
      }
    } catch (error) {
  ...
  return (
      ...
      <div className="min-h-screen animatedBackground bg-gray-100 p-8">
        ...
        {user && (
          <>
            <button onClick={() => setShowModal(true)} className="bg-blue-500 mb-4 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
              Ajouter un bon plan
            </button>
            {showModal && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                <div className="bg-white p-6 rounded">
                  <form onSubmit={submitNewBonPlan} className="space-y-4">
                    <input type="text" name="Titre" value={newBonPlan.Titre} onChange={handleInputChange} placeholder="Titre" required className="w-full p-2 border rounded" />
                    <textarea name="Description" value={newBonPlan.Description} onChange={handleInputChange} placeholder="Description" required className="w-full p-2 border rounded" />
                    <input type="url" name="imglink" value={newBonPlan.imglink} onChange={handleInputChange} placeholder="URL de l'image" className="w-full p-2 border rounded" />
                    <input type="url" name="LienAffiliation" value={newBonPlan.LienAffiliation} onChange={handleInputChange} placeholder="Lien d'affiliation" required className="w-full p-2 border rounded" />
                    <div className="flex justify-between items-center">
                      <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        Soumettre
                      </button>
                      <button onClick={() => setShowModal(false)} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-2 rounded">
                        Fermer
                      </button>
                    </div>
                  </form>
        ...
        {bonPlans.length > 0 ? (
          bonPlans.map((bonPlan, index) => (
            <div
              ...
              shadow p-4 hover:shadow-md transition-shadow"
            >
              <img
              ...
              <Link
                ...
                className=
                "text-indigo-500 hover:text-indigo-600 transition-colors mt-2 inline-block"
              >
                Voir plus
              </Link>
            </div>
